...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
		...
		LDR		R1, =SSI0_SR_R
cLoop	LDRB	R2, [R1] ; read SSI0_SR_R
		AND		R2, #0x10
		CMP		R2, #0x10 ; check bit 4
		BEQ		cLoop ; bit 5 is high (wait for busy bit to be 0)
		
		LDR		R1, =GPIO_PORTA_DATA_R
		LDRB	R2, [R1] ; Read port A
		BIC		R2, #0x40 ; clear PA6 to 0 to set D/C pin to command
		STRB	R2, [R1]
		
		LDR		R1, =SSI0_DR_R
		STRB	R0, [R1] ; write command to SSIO_R_R
		
		
		LDR		R1, =SSI0_SR_R ; //to double check command has been processed
cLoop2	LDRB	R2, [R1] ; read SSI0_SR_R again
		AND		R2, #0x10
		CMP		R2, #0x10 ; check bit 4 again
		BEQ		cLoop2 ; bit 5 is hight (wait for busy bit to be 0)
...
;4) Write the 8-bit data to SSI0_DR_R
		...
		LDR		R1, =SSI0_SR_R 
dLoop	LDRB	R2, [R1] ; read SSIO_SR_R
		AND		R2, #0x02 
		CMP		R2, #0x00 ; check bit 1
		BEQ		dLoop ; if bit 1 is 0, loop back
		
		LDR		R1, =GPIO_PORTA_DATA_R
		LDRB	R2, [R1] ; read Port A
		ORR		R2, #0x40 ; set PA6 to 1 to set D/C pin to data
		STRB	R2, [R1]
		
		LDR		R1, =SSI0_DR_R
		STRB	R0, [R1] ; write 8-bit data to SSIO_DR_R
